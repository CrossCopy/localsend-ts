...
export class DenoClient {
	...
	async get<TData = unknown, TError = unknown, ThrowOnError extends boolean = boolean>(
		options: RequestOptions<ResponseStyle, ThrowOnError, string>
	): Promise<RequestResult<TData, TError, ThrowOnError, ResponseStyle>> {
		...
		try {
			...
			if (!response.ok) {
				return {
					...
					response
				} as unknown as RequestResult<TData, TError, ThrowOnError, ResponseStyle>
			}
