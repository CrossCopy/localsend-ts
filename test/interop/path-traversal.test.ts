...
test("rejects empty filename instead of writing a sibling of saveDirectory", async () => {
	...
	try {
		...
		const prep = await fetch(`${base}/prepare-upload`, {
			...
			body: JSON.stringify({
				info,
				files: { f1: { id: "f1", fileName: "", size: 5, fileType: "text/plain" } }
			})
		})
		...
		// must not silently succeed while writing outside saveDirectory
		expect(up.status).toBeGreaterThanOrEqual(400)
		const parentDir = path.resolve(receiver.saveDir, "..")
		const saveDirName = path.basename(receiver.saveDir)
		const escapedSibling = path.join(parentDir, `${saveDirName} (1)`)
...
test("rejects '.' filename instead of writing a sibling of saveDirectory", async () => {
	const receiver = await startReceiver({ autoAccept: true })
	try {
		const base = `http://127.0.0.1:${receiver.port}/api/localsend/v2`
		const info = {
